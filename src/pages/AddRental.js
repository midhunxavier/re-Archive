    ...
    if (data.network == networksMap[networkDeployedTo]) {
      if (image !== undefined && window.ethereum !== undefined) {
        try {
          ...
          const listingFee = reArchiveContract.callStatic.listingFee();

          const cid = await StoreContent(image);
          const imageURI = `ipfs://${cid}/${imageName}`;

          //const imageURI = `https://${cid}.ipfs.w3s.link//?filename=${imageName}`;

          console.log(imageURI);

          const add_tx = await reArchiveContract.addRental(
            formInput.name,
            formInput.city,
            formInput.latitude,
            formInput.longitude,
