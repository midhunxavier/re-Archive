...
import logo from "../assets/images/LR.png";
...
import DecentralreArchive from "../artifacts/DecentralreArchive.sol/DecentralreArchive.json";
...
const Rentals = () => {
  ...
  const getRentalsList = async () => {
    ...
    const reArchiveContract = new ethers.Contract(
      contractAddress,
      DecentralreArchive.abi,
      signer
    );

    const rentals = await reArchiveContract.getRentals();
  ...
  const bookProperty = async (_id, _price) => {
    if (data.network == networksMap[networkDeployedTo]) {
      try {
        ...
        const totalBookingPriceETH = await reArchiveContract.convertFromUSD(
          utils.parseEther(totalBookingPriceUSD.toString(), "ether")
        );

        const book_tx = await reArchiveContract.bookDates(
          _id,
          _datefrom,
          _dateto,
