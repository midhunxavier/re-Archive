...
contract DecentralreArchive is PriceConverter {
    //--------------------------------------------------------------------
    // VARIABLES
    ...
    error DecentralreArchive__OnlyAdmin();
    error DecentralreArchive__InvalidFee();
    error DecentralreArchive__InvalidRentalId();
    error DecentralreArchive__InvalidBookingPeriod();
    error DecentralreArchive__AlreadyBooked();
    error DecentralreArchive__InsufficientAmount();
    error DecentralreArchive__TransferFailed();

    //--------------------------------------------------------------------
    // MODIFIERS

    modifier onlyAdmin() {
        if (msg.sender != admin) revert DecentralreArchive__OnlyAdmin();
        _;
    }

    modifier isRental(uint _id) {
        if (_id >= _rentalIds) revert DecentralreArchive__InvalidRentalId();
        _;
    }
    ...
    function addRental(
        ...
        if (msg.value != listingFee) revert DecentralreArchive__InvalidFee();
    ...
    function bookDates(
        ...
        if (bookingPeriod < 1)
            revert DecentralreArchive__InvalidBookingPeriod();

        uint256 _amount = convertFromUSD(_rental.pricePerDay) * bookingPeriod;

        if (msg.value != _amount)
            revert DecentralreArchive__InsufficientAmount();
        if (checkIfBooked(_id, _fromDateTimestamp, _toDateTimestamp))
            revert DecentralreArchive__AlreadyBooked();
        ...
        if (!success) revert DecentralreArchive__TransferFailed();
